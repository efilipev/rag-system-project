...
class OpenAIProvider(BaseLLMProvider):
    ...
    Follows Single Responsibility Principle - handles only OpenAI-specific logic
    """
    ...
    def __init__(self, api_key: str, default_model: str = "gpt-3.5-turbo"):
        ...
        logger.info(f"OpenAIProvider initialized with model: {default_model}")
    ...
    def _validate_token_count(
        ...
        for key in self.context_limits:
            if model.startswith(key):
                context_limit = self.context_limits[key]
                break
